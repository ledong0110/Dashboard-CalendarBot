...
const listEvents = async (auth, email) => {
  date = new Date();
  maxDate = new Date();
  maxDate.setDate(date.getDate() + 30); // NUMBER MEANS HOW MANY DAYS TO CONSIDER
...
router.get("/infoSetting", async(req, res) => {
  User.findOne({
    _id: req.user._id,
  })
  .then((user) => {
    ...
    return res.status(200).send({
      telegramUsername: user.telegramAccount,
      authenticated
    })
  })
});
...
router.post("/telegram", async(req, res) => {
  ...
  User.findOneAndUpdate(
    ...
    {
      telegramAccount: req.body.telegramUsername,
    }
  )
    .then((doc) => {
